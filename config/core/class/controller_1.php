...
class Controller implements ArrayAccess
{
     ...
     */    
    public final function __invoke($name, $value = null, $arg = null)
    {
    ...
    public function doSessionStart($value = null)
    {
        $sid    = \session_id();
        $sname  = \session_name();
        
        if (empty($sid) && !empty($value)) {
            $sname = 'sid';
            if (is_string($value) && !empty($value))
                $sname = $value;
     ...
     * @var
     */
    public function doStringLogger($value)
    {
        $trace = debug_backtrace();
        $caller = $trace[0];
        logger(array("{$caller['file']} @line {$caller['line']}", $value), $this->class);
