...
var Calendar = new Class({	
	...
	initialize: function(obj, options) {
		...
		this.fx = this.calendar.morph('opacity', { 
			onStart: function() { 
	...
	element: function(el, f, cal) {
		...
		if (el.getElement() == 'select') { // select elements allow the user to manually set the date via select option
	...
	rebuild: function(cal) {
		cal.els.each(function(el) {			
			/*
			if (el.getElement() == 'select' && el.format.test('^(F|m|M|n)$')) { // special case for months-only select
			...
			*/
			...
			if (el.getElement() == 'select' && el.format.test('^(d|j)$')) { // special case for days-only select
